...
import di.uniba.map.b.adventure.games.PrisonBreak;
import di.uniba.map.b.adventure.parser.Parser;
import di.uniba.map.b.adventure.parser.ParserOutput;
import di.uniba.map.b.adventure.type.AdvObject;
import di.uniba.map.b.adventure.type.Room;
import di.uniba.map.b.adventure.rest.Weather;
...
public final class EngineGUI extends javax.swing.JFrame {
    ...
    private static EngineGUI instance;
    ...
    /**
     * Attributo d'istanza di inizializzazione Utils.
     */
    private Utils timerThread;

    /**
     * Costruttore di EngineGUI.
     *
     * Questo costruttore crea un'istanza della classe EngineGUI e inizializza
     * il gioco PrisonBreak. Successivamente, viene chiamato il metodo
     * initializeGui() per inizializzare l'interfaccia grafica.
     */
    public EngineGUI() {
        initComponents();
        this.game = new PrisonBreak();
        initializeGui();
        instance = this;
    }
     ...
     *
     * Questo metodo gestisce l'inizializzazione dell'interfaccia grafica del
     * gioco. Controlla se è stato caricato un gioco precedente o se è un nuovo
     * gioco e in base a ciò configura l'interfaccia grafica di conseguenza.
     */
    private void initializeGui() {
        ...
        if (!GameDescription.loadedGame) {
            ...
            timerThread = new Utils(Database.getRemainingTime()); // Imposta il timer con i minuti rimanenti
            timerThread.start();
    ...
    private void initComponents() {
        ...
        InventaryButton = new javax.swing.JMenu();
        ...
        getContentPane().add(display, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 70, 1085, 345));
        ...
        getContentPane().add(inputArea, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 420, 1080, 63));
        ...
        getContentPane().add(roomsDisplay, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 10, 1080, -1));
        ...
        getContentPane().add(startButton, new org.netbeans.lib.awtextra.AbsoluteConstraints(1130, 190, 129, 49));

        textPolice.setVisible(false);
        textPolice.setFont(new java.awt.Font("Noto Sans", 0, 18)); // NOI18N
        textPolice.setText("Arrivo delle guardie tra:");
        getContentPane().add(textPolice, new org.netbeans.lib.awtextra.AbsoluteConstraints(1100, 10, 210, -1));

        time.setFont(new java.awt.Font("Noto Sans", 0, 18)); // NOI18N
        getContentPane().add(time, new org.netbeans.lib.awtextra.AbsoluteConstraints(1100, 40, 210, 30));
        ...
        InventaryButton.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                InventaryButtonMouseClicked(evt);
            }
        });
        menuBar.add(InventaryButton);
     ...
     *
     * Questo metodo gestisce l'azione scatenata dall'area di input quando viene
     * premuto il tasto "Invio". Prende il testo inserito nell'area di input, lo
     * processa utilizzando il parser e il gioco corrente, quindi esegue la
     * mossa successiva del gioco in base al comando analizzato.
     */
  private void inputAreaActionPerformed(final java.awt.event.ActionEvent evt) {//GEN-FIRST:event_inputAreaActionPerformed
      ...
      try {
          ...
          if (p.getCommand() != null) { // Se ho trovato il comando
              ...
              displayText.setText("Comando non valido");
          }
  ...
  private void roomsDisplayActionPerformed(final java.awt.event.ActionEvent evt) {//GEN-FIRST:event_roomsDisplayActionPerformed
    ...
    /**
     * Azione del pulsante di avvio. Questo metodo gestisce l'evento generato
     * dal pulsante di avvio quando viene premuto. Imposta il timer del gioco,
     * abilita i pulsanti e gli elementi dell'interfaccia grafica, quindi mostra
     * la descrizione della stanza attuale.
     */
  private void startButtonActionPerformed(final java.awt.event.ActionEvent evt) {//GEN-FIRST:event_startButtonActionPerformed
      int cod = Weather.getWeatherCode(); //Verifica il meteo presso la prigione di Fox River, in base al quale viene stabilito il tempo a disposizione
      int minute;
      if ((cod < 5)) {
          minute = 20;
      } else {
      ...
      String msg = (cod < 5) ? "Per tua fortuna tutto sommato è una bella giornata. Forse le guardie se la prenderanno comoda" : "Oggi c'è un meteo terribile. Probabilmente le guardie torneranno prima";
      
      timerThread = new Utils(minute * 60); // Imposta il timer in minuti
      timerThread.start();
      fileButton.setEnabled(true);
      time.setVisible(true);
      textPolice.setVisible(true);
      ...
      inputArea.setVisible(true);
      roomsDisplay.setVisible(true);
      displayText.setText(game.getCurrentRoom().getDescription() + "\n" + "Condizioni meteorologiche: " + Weather.getWeatherDescription(cod) + "\n" + msg); // Descrizione della stanza attuale
  }//GEN-LAST:event_startButtonActionPerformed
    /**
     * Azione del click del pulsante di aiuto.
     *
     * Questo metodo gestisce l'azione scatenata dal click del pulsante di
     * aiuto. Riproduce un effetto sonoro, carica il testo di aiuto da un file e
     * mostra un messaggio di dialogo con il testo di aiuto come contenuto.
     */
  ...
  private void exitButtonActionPerformed(final java.awt.event.ActionEvent evt) {//GEN-FIRST:event_exitButtonActionPerformed
    ...
    /**
     * Azione del click del pulsante di uscita. Questo metodo gestisce l'azione
     * scatenata dal click del pulsante di uscita. Riproduce un effetto sonoro e
     * termina l'applicazione chiudendo il programma.
     */
  private void exitButtonMouseClicked(final java.awt.event.MouseEvent evt) {//GEN-FIRST:event_exitButtonMouseClicked
     ...
     *
     * Questo metodo gestisce l'azione scatenata dal click del pulsante di
     * riavvio del gioco. Interrompe la riproduzione della musica di sottofondo,
     * chiude la finestra corrente e apre una nuova finestra del menu
     * principale.
     */
  private void restartGameButtonActionPerformed(final java.awt.event.ActionEvent evt) {//GEN-FIRST:event_restartGameButtonActionPerformed
     ...
     *
     * Questo metodo gestisce l'azione scatenata dal click del pulsante di
     * salvataggio. Riproduce un effetto sonoro di pulsante, elimina i dati del
     * giocatore dal database, salva l'ID e il nome della stanza corrente
     * insieme al tempo rimanente nel database, e visualizza un messaggio di
     * conferma o di errore a seconda dell'esito del salvataggio.
     *
  ...
  private void saveButtonActionPerformed(final java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonActionPerformed
    ...
    private void InventaryButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_InventaryButtonMouseClicked
        Music.playEffect(".//resources//soundEffects//button.wav");
    ...
    }//GEN-LAST:event_InventaryButtonMouseClicked
    ...
    private javax.swing.JMenu InventaryButton;
     ...
     *
     *
     * @param s La String da aggiungere al testo presente nella variabile
     * 'displayText'
     */
    ...
    /**
     *
     * Stampa l'intro lettera per lettra in maniera ritardata, grazie
     * all'utilizzo del timer il ritardo è di 100 millisecondi. Inoltre viene
     * riprodotto un effetto sonoro.
     *
     * @param s La String da sostituire al testo presente nella variabile
     * 'displayText'
     */
     ...
     *
     * Questo metodo visualizza gradualmente il testo specificato nell'area di
     * visualizzazione con un ritardo tra le lettere. Viene utilizzato un timer
     * per gestire il ritardo tra l'apparizione di ogni lettera. Una volta che
     * tutte le lettere sono state visualizzate, l'area di input viene
     * abilitata, il cursore viene posizionato all'inizio del testo e l'area di
     * input ottiene il focus.
     *
    ...
    /**
     * Metodo gestione del delay per la stampa di uno dei due finali presenti
     * nel gioco.
     *
    ...
    /**
     * Questo metodo aggiunge il valore di 's' al testo della variabile
     * 'displayText'.
     *
     * @param s La String da aggiungere al testo presente nella variabile
     * 'displayText'
     ...
     *
     * Questo metodo visualizza il tempo rimanente nel gioco in base al testo
     * fornito come input. Il testo rappresenta il numero di secondi rimanenti.
     * Il metodo calcola il numero di minuti e i secondi rimanenti a partire dal
     * testo fornito e imposta il testo corrispondente nel componente "time"
    ...
    /**
     * Metodo per disabilitare la inputArea nel caso in cui il giocatore non
     * deve poter inserire dati in input.
     */
    ...
    /**
     * Nasconde le etichette dell'interfaccia grafica. Questo metodo nasconde le
     * varie labels dell'interfaccia grafica, incluso il tempo. Infine, viene
     * 'ripulito' il display.
     */
    ...
    /**
     * Metodo d'istanza che permette il ritorno al menù quando il tempo a
     * disposizione termina.
     *
     * @param text
     */
    ...
    /**
     * Metodo d'istanza per impostare il numero di secondi necessari al ritorno
     * al menù principale nel caso in cui il tempo a disposizione termini.
     *
     */
    ...
    /**
     * Questo metodo ricerca l'id presente nel database in tutte le stanze
     * presenti nel gioco, se trova una corrispondenza di id con un oggetto
     * d'istanza di Room, ritorna l'id dell'oggetto dell'istanza. Null
     * altrimenti.
     *
    ...
    /**
     * Metodo pubblico per modificare il timer
     *
     * @param newTime
     */
    public void modifyTimer(int newTime) {
        if (timerThread != null) {
            Utils.modifyTimer(newTime);
        }
    }

    /**
     * Metodo per ottenere l'istanza unica di EngineGUI
     *
     * @return instance
     */
    public static EngineGUI getInstance() {
        if (instance == null) {
            instance = new EngineGUI(); // Se non esiste, la crea
        }
        return instance;
    }
}
